...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-apps-script-type-slides.git --from 0.0.0-preview
```

Then add `GoogleAppsScriptTypeSlides` to your target's dependencies:

```bash
swift package add-target-dependency GoogleAppsScriptTypeSlides <target-name> --package swift-google-apps-script-type-slides
```
